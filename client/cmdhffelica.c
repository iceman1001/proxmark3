...
int CmdHFFelicaSniff(const char *Cmd) {
	char * pEnd;
	UsbCommand c = {CMD_FELICA_SNOOP, {strtol(Cmd, &pEnd,0),strtol(pEnd, &pEnd,0),0}};
    clearCommandBuffer();
	SendCommand(&c);
	return 0;
	/*int param = 0;	
...
int CmdHFFelicaSimLite(const char *Cmd)
{
	...
	UsbCommand c = {CMD_FELICA_LITE_SIM, {(val&0xffffffff),(val>>32),0}};
	SendCommand(&c);
	return 0;
}
...
{
if(tracepos+19>=tracelen) return tracelen;
trace+=tracepos;
...
case 0x0E: {
         ...
         line[0]=0;
         for (int j = 0; j < 8; j++) snprintf(line+( j  * 2),110, "%02x", trace[j+11]);
         PrintAndLog( "REG: regA: %d regB: %d regC: %s ",regA,regB,line);
         }break;
case 0x80: {
            PrintAndLog( "Random Challenge, WO:  %s ",line);
           }break;  
case 0x81: {
            PrintAndLog( "MAC, only set on dual read:  %s ",line);
           }break;            
case 0x82: {
         char idd[20];
         char idm[20];
         for (int j = 0; j < 8; j++) snprintf(idd+( j  * 2),20, "%02x", trace[j+3]);
          for (int j = 0; j < 6; j++) snprintf(idm+( j  * 2),20, "%02x", trace[j+13]);
            PrintAndLog( "ID Block, IDd: 0x%s DFC: 0x%02x%02x Arb: %s ",idd,trace[11], trace [12],idm);
           }break;         
case 0x83: {
         char idm[20];
         char pmm[20];
         for (int j = 0; j < 8; j++) snprintf(idm+( j  * 2),20, "%02x", trace[j+3]);
          for (int j = 0; j < 8; j++) snprintf(pmm+( j  * 2),20, "%02x", trace[j+11]);
             PrintAndLog( "DeviceId:  IDm: 0x%s PMm: 0x%s ",idm,pmm);
           }break;    
  case 0x84: {
            PrintAndLog( "SER_C: 0x%02x%02x ",trace[3],trace[4]);
           }break;   
           
     case 0x85: {
            PrintAndLog( "SYS_Cl 0x%02x%02x ",trace[3],trace[4]);
           }break;   
  case 0x86: {
            PrintAndLog( "CKV (key version): 0x%02x%02x ",trace[3],trace[4]);
           }break;  
case 0x87: {
            PrintAndLog( "CK (card key), WO:   %s ",line);
           }break;         
case 0x88: {
            PrintAndLog( "Memory Configuration (MC):");
            PrintAndLog( "MAC needed to write state: %s", trace[3+12]? "on":"off");
            //order might be off here...
            PrintAndLog("Write with MAC for S_PAD : "BYTE_TO_BINARY_PATTERN" "BYTE_TO_BINARY_PATTERN" ",BYTE_TO_BINARY(trace[3+10]), BYTE_TO_BINARY(trace[3+11]));
            PrintAndLog("Write with AUTH for S_PAD : "BYTE_TO_BINARY_PATTERN" "BYTE_TO_BINARY_PATTERN" ",BYTE_TO_BINARY(trace[3+8]), BYTE_TO_BINARY(trace[3+9]));
            PrintAndLog("Read after AUTH for S_PAD : "BYTE_TO_BINARY_PATTERN" "BYTE_TO_BINARY_PATTERN" ",BYTE_TO_BINARY(trace[3+6]), BYTE_TO_BINARY(trace[3+7]));
           ...
           PrintAndLog("RW access for S_PAD : "BYTE_TO_BINARY_PATTERN" "BYTE_TO_BINARY_PATTERN" ",BYTE_TO_BINARY(trace[3+0]), BYTE_TO_BINARY(trace[3+1]));
...
default:  PrintAndLog( "INVALID %d: %s",blocknum, line);break;       
}
...
static command_t CommandTable[] = {
  ...
  // to be replaced
  {"simlite",    CmdHFFelicaSimLite,       0, "<NDEF2> - only reply to poll request"},
  {"dmp",    CmdHFFelicaDumpLite,       0, "Wait for and try dumping FelicaLite"},
